...
public class Support {
	...
	public static void init() {
		if (!commonInitialized) {
			DomGlobal.console.trace("common initialization");
					...
					if (xhr.getReadyState() == XMLHttpRequest.DONE) {
						if (xhr.getStatus() == 200) {
							if (!tzTnitialized && !tzTnitializing) {
								tzTnitializing = true;
								DomGlobal.console.trace("tz asynch initialization");
...
//								ZoneRulesProvider.registerProvider(provider);
							...
							DomGlobal.console.error("Can't load TZDB asynch. Response status: {} {}", xhr.getStatus(),
									xhr.getStatusText());
						}
					}
				}
	...
	public static void initTzData() {
		if (!tzTnitialized && !tzTnitializing) {
			tzTnitializing = true;
			DomGlobal.console.trace("tz synch initialization");
